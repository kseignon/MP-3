...
trail_data_fun <- function(data1, trail_name) {
  ...
  new_df2 <- left_join(new_df, st_geometry(data1))
...
factpal2 <- colorFactor(topo.colors(4), trail_df_sf4$category)
...
leaflet() %>%
 ...
 addPolylines(data = macleish_layers[["boundary"]])%>%
 addPolylines(data = Boundary_contours) %>%
 addPolylines(data = trail_df_sf4, color= ~factpal3(category))
