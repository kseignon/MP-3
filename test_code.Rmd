...
trail_df_as_sp <- mapply(trail_df_list, n = n_list, list_to_df_fun)

#trail_df <- merge(unlist(trail_df_list)) #only resulted in one row, names are gone...
trail_df <- Reduce(function(...) merge(..., all=T), as(trail_df_list, "Spatial"))

n_list <- c(1, 2, 3, 4, 5, 6)

list_to_df_fun <- function(item1, n){
  return(item1[[n]])
}
